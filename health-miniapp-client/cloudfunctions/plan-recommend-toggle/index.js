...
exports.main = async (event, context) => {
  ...
  const next = !doc.data.isRecommended
  await db.collection('health_plan').doc(id).update({
    data: { isRecommended: next, updatedAt: db.serverDate() }
  })
  return { success: true, data: { isRecommended: next } }
}
